		...
		if(is_array($found)) {
			...
			for($i = $start; $i < $end; ++$i) {
				...
				$ajaxArray['rows'][] = array(
					'<img src="https://cravatar.eu/helmavatar/'.$playerName.'/23" alt="'.$playerName.'" class="minihead" /> <a href="index.php?action=viewplayer&player='.$playerName.'&server='.$_GET['server'].'">'.$playerName.'</a>',
					$player['type'],
