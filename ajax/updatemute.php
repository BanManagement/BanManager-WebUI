	...
	if(!isset($error)) {
		...
		if(!$mysqlicon)
			...
			if(mysqli_num_rows($currentMute) == 0)
				...
				mysqli_query($mysqlicon, "UPDATE ".$server['playerMutesTable']." SET reason = '".$_POST['reason']."', updated = UNIX_TIMESTAMP(now()),  expires = '$timestamp', soft = '$soft', silent = '$silent' WHERE id = '".$_POST['id']."'");
