...
if(!isset($_SESSION['admin']) || (isset($_SESSION['admin']) && !$_SESSION['admin']))
	...
	if(!$mysqlicon)
		...
		if(mysqli_num_rows($currentMute) == 0) {
			...
			mysqli_query($mysqlicon, "INSERT INTO ".$server['playerMuteRecordsTable']." (player_id, reason, expired, actor_id, pastActor_id, pastCreated, created, createdReason, soft, silent) SELECT b.player_id, b.reason, b.expires, UNHEX('$consoleId'), b.actor_id, b.created, UNIX_TIMESTAMP(now()), 'WebUI', b.soft, b.silent FROM ".$server['playerMutesTable']." AS b WHERE b.id = '".$_GET['id']."'");
			// Now delete it
