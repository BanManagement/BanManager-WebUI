...
if(!isset($_SESSION['admin']) || (isset($_SESSION['admin']) && !$_SESSION['admin']))
	...
	if(!$mysqlicon)
		...
		$pastBans = mysqli_query($mysqlicon, "SELECT warn_id FROM ".$server['playerWarningsTable']." WHERE warn_id = '".$_GET['id']."'");

		if(mysqli_num_rows($pastBans) == 0)
			$error = 'That record does not exist';
		else {
			mysqli_query($mysqlicon, "DELETE FROM ".$server['playerWarningsTable']." WHERE warn_id = '".$_GET['id']."'");
