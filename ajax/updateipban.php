...
if(!isset($_SESSION['admin']) || (isset($_SESSION['admin']) && !$_SESSION['admin']))
	...
	if(!isset($error)) {
		...
		if(!$mysqlicon)
			...
			$currentBan = mysqli_query($mysqlicon, "SELECT id FROM ".$server['ipBansTable']." WHERE id = '".$_POST['id']."'");

			if(mysqli_num_rows($currentBan) == 0)
				$error = 'That ban does not exist';
			else {
				mysqli_query($mysqlicon, "UPDATE ".$server['ipBansTable']." SET reason = '".$_POST['reason']."', updated = UNIX_TIMESTAMP(now()), expires = '$timestamp' WHERE id = '".$_POST['id']."'");
