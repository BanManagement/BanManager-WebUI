...
export default function Modal ({ open = false, icon, title, children, confirmButton, confirmDisabled, onCancel, onConfirm, loading }) {
  ...
  return (
    <Dialog open={open} transition className='relative inset-0 overflow-y-auto' initialFocus={cancelButtonRef} onClose={onClose}>
      ...
      />
      <div className='fixed inset-0 flex w-screen items-center justify-center overflow-y-auto p-4 text-gray-200'>
        <div className='flex min-h-full items-center justify-center w-full max-w-md'>
          <DialogPanel
            transition
