const PageHeader = ({ title, subTitle, containerClassName = '', className = '', children, ...rest }) => {
  return (
    <>
      ...
      <div className={`flex justify-center items-center pb-4 mb-4 border-b border-accent-400 ${containerClassName}`}>
        <h1 className={`text-3xl font-bold leading-none ${className}`} {...rest}>{title}</h1>
        {children}
      </div>
    </>
  )
