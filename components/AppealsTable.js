...
const query = `query listPlayerAppeals($serverId: ID, $actor: UUID, $assigned: UUID, $state: ID, $limit: Int, $offset: Int) {
  listPlayerAppeals(serverId: $serverId, actor: $actor, assigned: $assigned, state: $state, limit: $limit, offset: $offset) {
    total
    records {
      id
      created
...
export default function AppealsTable ({ limit = 30 }) {
  const [tableState, setTableState] = useState({ serverId: null, activePage: 1, limit, offset: 0, actor: null, assigned: null, state: null })
  ...
  const rows = data?.listPlayerAppeals?.records || []
    ...
    <Table selectable>
      <Table.Header>
        <Table.Row>
          ...
          <Table.HeaderCell><PlayerSelector multiple={false} clearable placeholder='Appealing' handleChange={handleFieldChange('actor')} /></Table.HeaderCell>
        ...
        {loading
          ...
          : rows.map((row, i) => (
            <Table.Row key={i}>
              ...
              <Table.Cell><a href={`/appeals/${row.id}`}>#{row.id}</a></Table.Cell>
              <Table.Cell>
