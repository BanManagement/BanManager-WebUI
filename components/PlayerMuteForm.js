...
class PlayerMuteForm extends React.Component {
  handleChange = (e, data) => {
  ...
  constructor(props) {
    ...
    const { data: { id, created, reason, expires, player, soft } } = props
    let servers = null
  ...
  render() {
    const { created, expires, reason, player, servers, server, error, loading, expiryType, soft } = this.state
