...
const PlayerMuteRow = ({ row, dateFormat, serverId, onDeleted }) => {
  const [open, setOpen] = useState(false)

  const { load, data, loading, errors } = useMutateApi({
    query: `mutation deletePlayerMuteRecord($id: ID!, $serverId: ID!) {
          ...
          {row.acl.delete &&
            <>
              ...
              >
                ...
                <p className='pb-1'>Are you sure you want to delete this mute record?</p>
...
export default function PlayerMutes ({ id, color, limit = 10 }) {
  const [tableState, setTableState] = useState({ type: 'PlayerMuteRecord', activePage: 1, limit, offset: 0, player: id, serverId: null })
        ...
        <Table>
          <Table.Header>
            <Table.Row>
              ...
              <Table.HeaderCell>Unmuted By</Table.HeaderCell>
