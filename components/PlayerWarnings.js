...
const PlayerWarningRow = ({ row, dateFormat, serverId, onDeleted }) => {
  const [open, setOpen] = useState(false)

  const { load, data, loading, errors } = useMutateApi({
    query: `mutation deletePlayerWarning($id: ID!, $serverId: ID!) {
  ...
  return (
    <Table.Row>
      ...
      <Table.Cell><p>{format(fromUnixTime(row.created), dateFormat)}</p></Table.Cell>
      <Table.Cell>{row.points}</Table.Cell>
      <Table.Cell>{row.read && <TiTick className='w-5 h-5' />}</Table.Cell>
      <Table.Cell>
          ...
          {row.acl.delete &&
            <>
              ...
              >
                ...
                <p className='pb-1'>Are you sure you want to delete this warning?</p>
