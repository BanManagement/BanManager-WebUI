...
module.exports = async function listPlayerBans (obj, { serverId, actor, player, limit = 10, offset, order }, { state }, info) {
  ...
  if (fields.total) {
    const { total } = await server.pool(tables.playerBans)
  ...
  if (fields.records) {
    const query = getSql(info.schema, server, fields.records, 'playerBans')
      .where(filter)
      .limit(limit)
    ...
    if (fields.records.fieldsByTypeName.PlayerBan.acl) {
      calculateAcl = true
