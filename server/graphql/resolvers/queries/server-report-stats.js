...
module.exports = async function serverReportStats (obj, { id, intervalDays }, { state: { serversPool }, log }) {
  ...
  const { total } = await pool(config.tables.playerReports)
    .select(pool.raw('COUNT(*) AS `total`'))
    .where('created', '>=', startUnixTime)
    .first()
  const { averageLength } = await pool(config.tables.playerReports)
    .select(pool.raw('FLOOR(SUM(updated - created) / COUNT(*)) AS averageLength'))
    .where('created', '>=', startUnixTime)
    .where('state_id', '>=', 3)
    .first()
  const totalHistoryValues = await pool(config.tables.playerReports)
