...
module.exports = async function resolveAppealDeleteBan (obj, { id }, { session, state }, info) {
  ...
  const canDelete = state.acl.hasServerPermission(data.server_id, 'player.bans', 'delete.any') ||
  ...
  await server.pool.transaction(async trx => {
    const select = trx.select(
      ...
      .from(`${server.config.tables.playerBans} as b`)
      .where('b.id', data.punishment_id)

    await trx(
      trx.raw(
        '?? (??, ??, ??, ??, ??, ??, ??, ??, ??)', [
          server.config.tables.playerBanRecords,
          'player_id',
          'reason',
          'expired',
          'actor_id',
    ...
    await trx(server.config.tables.playerBans).where({ id: data.punishment_id }).del()
