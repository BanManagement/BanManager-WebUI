...
module.exports = async function resolveAppealUpdateWarning (obj, { id, input }, { session, state }, info) {
  ...
  const canUpdatePunishment = state.acl.hasServerPermission(data.server_id, 'player.warnings', 'update.any') ||
  ...
  await server.pool.transaction(async trx => {
    await trx(server.config.tables.playerWarnings).update({ expires: input.expires, reason: input.reason, points: input.points }).where({ id: data.punishment_id })
    ...
    return await state.dbPool.transaction(async trx => {
      ...
      if (punishment.points !== input.points) {
        comment.old_points = punishment.points
        comment.new_points = input.points ? 1 : 0
      }
